...
export const sessionChart = (sessions) => {
  ...
  // Add X axis
  const x = d3.scaleBand().domain(days).range([0, width]);
  svg
    .append("g")
  ...
  const y = d3
    ...
    .range([height, 50]);
  svg.append("g").attr("class", "axis_line--y").call(d3.axisLeft(y).ticks(7));
  ...
  const block = d3.select("#session").append("div").attr("class", "highlight");

  // Three function that change the tooltip when user hover / move / leave a cell
  const mouseover = function (event, d) {
    Tooltip.style("opacity", 1);

    block.style("opacity", 1).style("left", `${event.layerX + 5}px`);
  };
  ...
  const mouseleave = function (event, d) {
    ...
    block.style("opacity", 0).style("left", "100%");
  };
